...
public class Graph implements Serializable {
    ...
    // Get celebrity names for search (optimized with cached map)
    private transient volatile Map<String, String> cachedActorNames;
    private transient volatile List<Map.Entry<String, String>> searchableCelebrities;
    
    public Map<String, String> getActorNames() {
        if (cachedActorNames == null) {
            synchronized (this) {
                if (cachedActorNames == null) {
                    cachedActorNames = new HashMap<>();
                    searchableCelebrities = new ArrayList<>();
                    if (celebrityNames != null && celebrityIds != null) {
                        for (int i = 0; i < celebrityNames.length; i++) {
                            if (celebrityNames[i] != null && celebrityIds[i] != null) {
                                Map.Entry<String, String> entry = Map.entry(celebrityIds[i], celebrityNames[i]);
                                cachedActorNames.put(celebrityIds[i], celebrityNames[i]);
                                searchableCelebrities.add(entry);
                            }
                        }
                    }
                }
    ...
    // Fast search method that returns results directly (thread-safe, no copying)
    public List<Map.Entry<String, String>> searchCelebrities(String query, int maxResults) {
        if (searchableCelebrities == null) {
            getActorNames(); // Initialize if needed
        }
        
        List<Map.Entry<String, String>> results = new ArrayList<>();
        String lowerQuery = query.toLowerCase().trim();
        
        // Use the immutable list directly - no copying needed
        List<Map.Entry<String, String>> searchList = searchableCelebrities;
        
        for (Map.Entry<String, String> entry : searchList) {
    ...
    private void loadOrBuild() throws IOException {
        ...
        String cacheFile = "/app/graph-cache.bin";
        ...
        if (canUseCache) {
            try (ObjectInputStream in = new ObjectInputStream(new BufferedInputStream(Files.newInputStream(Paths.get(cacheFile)), 64 * 1024))) {
    ...
    private void saveCache(String cacheFile) {
        try {
            ...
            try (ObjectOutputStream out = new ObjectOutputStream(new BufferedOutputStream(Files.newOutputStream(cachePath), 64 * 1024))) {
